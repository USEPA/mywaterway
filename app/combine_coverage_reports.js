...
function copyFile(sourcePath, destinationPath, newFileName) {
    try {
        ...
        console.log(`Successfully copied ${sourcePath} to ${destinationPath} as ${newFileName}`);
    } catch (err) {
...
function createCombinedCoverageReport() {
    ...
    if (fs.existsSync(combinedCoverageDir)) {
        try {
            ...
            console.error(`Error removing combined_coverage_reports directory: ${err.message}`);
            process.exit(1);
        }
    }
    ...
    exec(`npx nyc report --reporter=html --temp-dir=${combinedCoverageDir} --report-dir=${combinedCoverageDir}/final_report`, (error, stdout, stderr) => {
        if (error) {
            console.error(`Error executing npx nyc report: ${error}`);
        } else {
            console.log(`Combined coverage report generated successfully. Please review ${combinedCoverageDir}/final_report`);
        }
    });
}
